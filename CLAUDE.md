...
✅ **21-sep-2026 — esta línea estrena las tres compuertas de quiz (ADR-077).** Era la única cuyo `build-course.js` no vigilaba el sesgo, y por eso nadie vio nunca que en `caracteristicas-educativas` la correcta era **la única más larga en 15 de 20 preguntas** (75 %, contra 27 % por azar). Los barridos anteriores medían **margen** —y ahí estaba limpio—; esto es **recuento**. Las tres se copiaron **tal cual** del build de Programa de Jóvenes para que no nazcan divergentes, **avisaron al primer build**, y el curso bajó a **8 de 20**. ⚠️ **Al tocar quizzes aquí, leer los avisos del build**: ahora los hay. Y al editar el build, no reescribir esas tres funciones: se sincronizan con PJ.

**21-sep-2026 — también:** 8 distractores corregidos por la auditoría doctrinal (tres eran **ERROR**: uno era paráfrasis casi literal de *Características Esenciales* p. 31), **60 erratas** publicadas —una en el **certificado**—, el «*Libro de la Selva*» que ni se llama así ni es el marco simbólico, dos elementos del Método con formulación **pre-2017**, y el mapa de ramas al que le faltaba la **Familia de Cachorros**. Su backend propio recibió el **ADR-074** (`recover` ya no entrega lo escrito) — ⚠️ **sin desplegar**: es un `clasp push` **y una promoción** aparte de los de la plataforma.
